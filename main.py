...
providers = [providers.romsgames, providers.romsfun]  #
    ...
    for game_b in json:
        if game_b["console_id"] == game["console_id"]:
            if list(game["url"].keys())[0] not in list(game_b["url"].keys()):
                aux_base = re.sub(
                    "[^A-Za-z0-9]+", "", game_b["name"]
                )  # adjusting the string of the game in base
                aux_game = re.sub(
                    "[^A-Za-z0-9]+", "", game["name"]
                )  # adjusting the string of the game that it's checking

                point = utils.similar(
                    aux_base, aux_game
                )  # getting the similarity value, can sometimes failed to get a good score
